...
export function buildHdrsSvg(m: HdrsSvgModel): string {
  ...
  const gridLines = [0.25, 0.5, 0.75].map(
    (f) => `<line x1="0" y1="${(H * f).toFixed(1)}" x2="${W}" y2="${(H * f).toFixed(1)}" stroke="${grid}"/>`,
  );
  const verticals = m.verticals.map((v) => {
    const line = `<line x1="${v.x.toFixed(1)}" y1="0" x2="${v.x.toFixed(
      1,
    )}" y2="${H}" stroke="#00f6d2" stroke-width="1.2"/>`;
    if (!v.label) return line;
    // Accent date pill at the bottom of the marker — mirrors the live chart.
    const w = v.label.length * 5.4 + 8;
    return (
      `${line}<rect x="${(v.x - w / 2).toFixed(1)}" y="${(H - 6).toFixed(1)}" width="${w.toFixed(
        1,
      )}" height="13" rx="3" fill="#00f6d2"/>` +
      `<text x="${v.x.toFixed(1)}" y="${(H + 3.5).toFixed(
        1,
      )}" text-anchor="middle" fill="#04222f" font-size="9" font-weight="600">${escapeXml(v.label)}</text>`
    );
  });
  const paths = m.series.map((s) => `<path d="${s.path}" fill="none" stroke="${s.color}" stroke-width="1.6"/>`);
  const xLabels = m.xLabels.map((t, i, arr) => {
    const anchor = i === 0 ? 'start' : i === arr.length - 1 ? 'end' : 'middle';
    return `<text x="${t.x.toFixed(1)}" y="${
      H + 16
    }" text-anchor="${anchor}" fill="${label}" font-size="10">${escapeXml(t.text)}</text>`;
  });
  ...
  m.series.forEach((s, i) => {
    ...
    axes.push(
      `<line x1="${x0}" y1="${Math.min(...ys).toFixed(1)}" x2="${x0}" y2="${Math.max(...ys).toFixed(1)}" stroke="${
        s.color
      }" stroke-width="1.5"/>`,
    );
    for (const a of s.axis) {
      axes.push(
        `<line x1="${x0}" y1="${a.y.toFixed(1)}" x2="${x0 + 5}" y2="${a.y.toFixed(1)}" stroke="${
          s.color
        }" stroke-width="1.5"/>` +
          `<text x="${x0 + 8}" y="${a.y.toFixed(1)}" fill="${
            s.color
          }" font-size="9" dominant-baseline="middle">${escapeXml(a.label)}</text>`,
      );
    }
  });
  ...
  const topPills = (m.topPills || []).map((p) => {
    ...
    return (
      `<rect x="${(p.x - w / 2).toFixed(1)}" y="2" width="${w.toFixed(1)}" height="13" rx="3" fill="#00f6d2"/>` +
      `<text x="${p.x.toFixed(
        1,
      )}" y="11.5" text-anchor="middle" fill="#04222f" font-size="9" font-weight="600">${escapeXml(p.text)}</text>`
    );
  });
  ...
  if (dt && tableRows > 0) {
    ...
    tableEls.push(
      `<text x="8" y="${yT + 12}" fill="${label}" font-size="10" font-weight="600">Series (${modeLabel})</text>`,
    );
    dt.columns.forEach((c, i) => {
      const head = (c.isBaseline ? '◎ ' : '') + (c.isToday ? 'today' : c.xLabel);
      tableEls.push(
        `<text x="${colRight(i).toFixed(1)}" y="${yT + 12}" text-anchor="end" fill="${
          c.isToday ? '#00f6d2' : label
        }" font-size="9" font-weight="600">${escapeXml(head)}</text>`,
      );
    });
    ...
    dt.rows.forEach((row, r) => {
      ...
      tableEls.push(
        `<text x="8" y="${ry.toFixed(1)}" fill="${row.series.color}" font-size="10" font-weight="600">${escapeXml(
          row.series.label,
        )}</text>`,
      );
      row.cells.forEach((cell, i) => {
        tableEls.push(
          `<text x="${colRight(i).toFixed(1)}" y="${ry.toFixed(
            1,
          )}" text-anchor="end" fill="#ffffff" font-size="10">${escapeXml(cell.text)}</text>`,
        );
        if (cell.deltaText) {
          ...
          tableEls.push(
            `<text x="${colRight(i).toFixed(1)}" y="${(ry + 11).toFixed(
              1,
            )}" text-anchor="end" fill="${dc}" font-size="8">${escapeXml(arrow + cell.deltaText + pct)}</text>`,
          );
        }
      });
    });
  ...
  const legend = m.series.map((s) => {
    ...
    const seg =
      `<line x1="${lx}" y1="31" x2="${lx + 12}" y2="31" stroke="${s.color}" stroke-width="2"/>` +
